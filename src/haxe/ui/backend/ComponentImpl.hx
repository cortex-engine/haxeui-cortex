...
import haxe.ui.backend.cortex.ScissorHelper;
...
class ComponentImpl extends ComponentBase {
    ...
    private function __onDoubleClick(event:MouseEvent) {
        ...
        if (i == true && button == 0) {
            ...
            _mouseDownFlag = false;
            var mouseDelta = (new Vec2(x, y) - new Vec2(_lastClickX, _lastClickY)).lengthSquared();
            if (_lastClickTimeDiff < 0.5 && mouseDelta < 5*5) { // 0.5 seconds
    ...
    public function draw() {
        ...
        StyleHelper.drawStyle(style, x, y, w, h, calcOpacity());
        
        if (_textDisplay != null) {
            _textDisplay.draw(Math.ceil(x), Math.ceil(y));
        }
        
        if (_textInput != null) {
            _textInput.draw(Math.ceil(x), Math.ceil(y));
        }
        
        if (_imageDisplay != null) {
            _imageDisplay.draw(Math.ceil(x), Math.ceil(y));
        }
